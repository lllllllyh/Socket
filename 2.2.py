...
class ClientApp(QWidget):
        ...
        if username and password:
            ...
            if self.protocol == "TCP":
                ...
                if response == "登录成功":
                    ...
                    threading.Thread(target=self.receive_message).start()  # 开启消息接收线程
                else:
    ...
    def receive_message(self):
        ...
        while True:
            try:
                ...
                if data:
                    self.log_area.append(f"📨 接收到消息: {data}")
            except:
                break
    ...
    def send_file(self):
        ...
        if file_name and self.client_socket:
            with open(file_name, 'rb') as file:  # 用二进制模式打开文件
                while True:
                    ...
                    if self.protocol == "TCP":
                        ...
                        self.client_socket.sendto(file_data, self.server_address)
            self.log_area.append(f"📁 文件发送完成: {file_name}")
